...
//                Store.event.point.imageUrl = choosenPoint["imageUrl"] as? URL ?? URL(string: "")
                let choosenPointIndex = snapshot.value as? Int
                Store.event.point = Store.nearbyPoints[choosenPointIndex!]
                Store.event.title = Store.nearbyPoints[choosenPointIndex!].name
